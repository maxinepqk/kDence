...
from openProj import (openProjMousePressed, openProjKeyPressed, 
openProjTimerFired, openProjRedrawAll)
...
def init(data):
    ...
    data.drawAudioTrackSpaces = [] # pos of last track
    ...
    # data.screenie = PhotoImage(file = "savedScreenies/1.gif")
    # number =  str(len([name for name in os.listdir("savedFiles") 
    #            if os.path.isfile(name)]))
    # for project in range(1, len(number)+1):
    #     data.screenie = PhotoImage(file = "savedScreenies/%s.gif"%str(project))
...
def mousePressed(event, data):
    ...
    elif (data.mode == "openProj"): openProjMousePressed(event, data)
    elif (data.mode == "replayingStart"): replayingStartMousePressed(event, data)
...
def keyPressed(event, data):
    ...
    elif (data.mode == "openProj"): openProjKeyPressed(event, data)
    elif (data.mode == "replayingStart"): replayingStartKeyPressed(event, data)
...
def timerFired(data):
    ...
    elif (data.mode == "recordEnd"): recordEndTimerFired(data)
    ...
    elif (data.mode == "openProj"): openProjTimerFired(data)
    elif (data.mode == "replayingStart"): replayingStartTimerFired(data)
...
def redrawAll(canvas, data):
    ...
    elif (data.mode == "openProj"): openProjRedrawAll(canvas, data)
    elif (data.mode == "replayingStart"): replayingStartRedrawAll(canvas, data)
...
def run(width=300, height=300): # adapted from course notes
    ...
    root = Tk()
    init(data) # changed placement of init to loadheight images
    # scrollbar code from http://bit.ly/2hgM3uJ
    frame=Frame(root,width=data.width,height=data.height)
    frame.grid(row=0,column=0)
    canvas=Canvas(frame,bg='#FFFFFF',width=data.width,height=data.height,
                  scrollregion=(0,0,1400,1400))
    hbar=Scrollbar(frame,orient=HORIZONTAL)
    hbar.pack(side=BOTTOM,fill=X)
    hbar.config(command=canvas.xview)
    canvas.config(width=data.width,height=data.height)
