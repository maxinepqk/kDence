...
def playAddDrumsMetroMousePressed(event, data):
    ...
    if (event.x > backx0 and event.x < backx1 and event.y > backy0 
        ...
        data.metronomeBPM -= 10
        if (data.metronomeBPM < 0): data.metronomeBPM = 0.1 # or else division by 0

def playAddDrumsMetroKeyPressed(event, data):
...
def playAddDrumsMetroTimerFired(data):
    data.drumMetroCounter += 1
    t = int((60 / data.metronomeBPM)*10)
    if (data.drumMetroCounter % t == 0):
...
def playAddDrumsMetroRedrawAll(canvas, data):
